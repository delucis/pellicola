...
async function FrameMaker (
  ...
  validate('FAANNSBO', [sketch, fonts, dimensions, totalFrames, fps, frameFormat, renderInParallel, motionBlur])
  ...
  async function drawWithMotionBlur (frame, canvas, context) {
    const { samplesPerFrame = 1, shutterAngle = 1 } = motionBlur
  ...
  return {
    ...
    getFrame (frame) {
      return new Promise(async (resolve, reject) => {
        ...
        try {
          if (motionBlur.samplesPerFrame > 1) {
