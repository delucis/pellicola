...
async function pellicola (
  ...
  const updateProgress = () => {
    ...
    if (!silent) progress.text = `Rendering frames (${done}/${totalFrames})`
  }
  ...
  if (renderInParallel) {
    ...
    await Promise.all(frames.map(async (_, frame) => writeFrame(frame).then(updateProgress)))
  } else {
