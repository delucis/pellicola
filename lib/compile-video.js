 ...
 * @param   {string}  [opts.outDir]    Path to write the output video to
 * @param   {string}  [opts.filename='out.mp4']  Filename of output video
 * @param   {number}  [opts.fps=defaultFrameRate]  Framerate of output video
 * @param   {'jpeg'|'png'}  [opts.frameFormat=defaultFrameFormat] Image format used for frames (png or jpeg)
 * @param   {number}  opts.startFrame Frame number the sequence starts on
...
async function compileVideo (
  ...
  return new Promise((resolve, reject) => {
    ...
    const ffmpeg = spawn(ffmpegPath, [
      ...
      '-start_number', startFrame.toString(), // start compiling from …00.png
